...
class LambdaRank(BaseEstimator):
    ...
    def fit(self, x, y, **fit_params):
        ...
        for i in range(self.epoch):
            ...
            for j in range(int(self.steps_per_epoch)):
                ...
                x1_cur = []
                y_cur = []
                idx0 = np.random.randint(0, len(x_epoch), self.scale_factor_train_sample * len(x_epoch))
                idx1 = np.random.randint(0, len(x_epoch), self.scale_factor_train_sample * len(x_epoch))
                x0_cur = np.array(x_epoch[idx0])
                x1_cur = np.array(x_epoch[idx1])
                y_cur = np.where(y[idx0] - y[idx1] >= 0, 1, -1).astype(np.float32)
                x_epoch = np.delete(x_epoch, np.append(idx0, idx1), 0)
                y_epoch = np.delete(y_epoch, np.append(idx0, idx1), 0)

                step, train_loss, score = self._train_step((x0_cur, x1_cur), y_cur)
                if step % 100 == 0:
    ...
    def predict_proba(self, features):
        ...
        res = self.model([features, np.zeros(np.shape(features))], batch_size=self.batch_size,
